...
GenfitMaterialInterface::GenfitMaterialInterface(const dd4hep::Detector* dd4hepGeo) {
...
GenfitMaterialInterface* GenfitMaterialInterface::getInstance(const dd4hep::Detector* dd4hepGeo) {
...
void GenfitMaterialInterface::destruct() { delete m_instance; }

TGeoManager* GenfitMaterialInterface::getGeoManager() { return m_geoManager; }

bool GenfitMaterialInterface::initTrack(double posX, double posY, double posZ, double dirX, double dirY, double dirZ) {
...
genfit::Material GenfitMaterialInterface::getMaterialParameters() {
  ...
  return genfit::Material(mat->GetDensity(), mat->GetZ(), mat->GetA(), mat->GetRadLen(), MeanExcEnergy_get(mat));
}

double GenfitMaterialInterface::findNextBoundary(const genfit::RKTrackRep* rep, const genfit::M1x7& stateOrig,
                                                 double sMax, // signed
                                                 bool varField) {
  ...
  while (1) {
    if (++it > maxIt) {
      genfit::Exception exc("GenfitMaterialInterface::findNextBoundary ==> maximum number of iterations exceeded",
                            __LINE__, __FILE__);
      exc.setFatal();
      throw exc;
    }
    ...
    if (slDist < delta) {
      if (debugLvl_ > 0)
        std::cout << "   very close to the boundary -> return"
                  << " stepSign*(s + slDist) = " << stepSign << "*(" << s + slDist << ")\n";
    ...
    if (debugLvl_) {
      std::cout << " RKTrackRep at state " << state7[0] << " " << state7[1] << " " << state7[2] << " " << state7[3]
                << " " << state7[4] << " " << state7[5] << " " << state7[6] << std::endl;
    }
    // Straight line distance² between extrapolation finish and
    // the end of the previously determined safe segment.
    double dist2 =
        (pow(state7[0] - oldState7[0], 2) + pow(state7[1] - oldState7[1], 2) + pow(state7[2] - oldState7[2], 2));
    // Maximal lateral deviation².
    double maxDeviation2 = 0.25 * (step * step - dist2);

    if (step > safety && maxDeviation2 > epsilon * epsilon) {
...
const double MeanExcEnergy_vals[MeanExcEnergy_NELEMENTS] = {
    1.e15, 19.2,  41.8,  40.0,  63.7,  76.0,  78.,   82.0,  95.0,  115.0, 137.0, 149.0, 156.0, 166.0, 173.0, 173.0,
    180.0, 174.0, 188.0, 190.0, 191.0, 216.0, 233.0, 245.0, 257.0, 272.0, 286.0, 297.0, 311.0, 322.0, 330.0, 334.0,
    350.0, 347.0, 348.0, 343.0, 352.0, 363.0, 366.0, 379.0, 393.0, 417.0, 424.0, 428.0, 441.0, 449.0, 470.0, 470.0,
    469.0, 488.0, 488.0, 487.0, 485.0, 491.0, 482.0, 488.0, 491.0, 501.0, 523.0, 535.0, 546.0, 560.0, 574.0, 580.0,
    591.0, 614.0, 628.0, 650.0, 658.0, 674.0, 684.0, 694.0, 705.0, 718.0, 727.0, 736.0, 746.0, 757.0, 790.0, 790.0,
    800.0, 810.0, 823.0, 823.0, 830.0, 825.0, 794.0, 827.0, 826.0, 841.0, 847.0, 878.0, 890.0,
};
// Logarithms of the previous table, used to calculate mixtures.
const double MeanExcEnergy_logs[MeanExcEnergy_NELEMENTS] = {
    34.5388, 2.95491, 3.7329,  3.68888, 4.15418, 4.33073, 4.35671, 4.40672, 4.55388, 4.74493, 4.91998, 5.00395,
    5.04986, 5.11199, 5.15329, 5.15329, 5.19296, 5.15906, 5.23644, 5.24702, 5.25227, 5.37528, 5.45104, 5.50126,
    5.54908, 5.6058,  5.65599, 5.69373, 5.73979, 5.77455, 5.79909, 5.81114, 5.85793, 5.84932, 5.8522,  5.83773,
    5.86363, 5.8944,  5.90263, 5.93754, 5.97381, 6.03309, 6.04973, 6.05912, 6.08904, 6.10702, 6.15273, 6.15273,
    6.1506,  6.19032, 6.19032, 6.18826, 6.18415, 6.19644, 6.17794, 6.19032, 6.19644, 6.21661, 6.25958, 6.28227,
    6.30262, 6.32794, 6.35263, 6.36303, 6.38182, 6.41999, 6.44254, 6.47697, 6.4892,  6.51323, 6.52796, 6.54247,
    6.5582,  6.57647, 6.58893, 6.60123, 6.61473, 6.62936, 6.67203, 6.67203, 6.68461, 6.69703, 6.71296, 6.71296,
    6.72143, 6.71538, 6.67708, 6.7178,  6.71659, 6.73459, 6.7417,  6.77765, 6.79122,
};
...
double GenfitMaterialInterface::getSafeDistance() { return getGeoManager()->GetSafeDistance(); }

double GenfitMaterialInterface::getStep() { return getGeoManager()->GetStep(); }

TGeoNode* GenfitMaterialInterface::findNextBoundary(double stepmax, const char* path, bool frombdr) {
  return getGeoManager()->FindNextBoundary(stepmax, path, frombdr);
}

bool GenfitMaterialInterface::isSameLocation(double posX, double posY, double posZ, bool change) {

  return getGeoManager()->IsSameLocation(posX, posY, posZ, change);
}

void GenfitMaterialInterface::setCurrentDirection(double nx, double ny, double nz) {
