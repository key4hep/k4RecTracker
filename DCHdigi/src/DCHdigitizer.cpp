...
DCHdigitizer::DCHdigitizer(const std::string& aName, ISvcLocator* aSvcLoc) : GaudiAlgorithm(aName, aSvcLoc), m_geoSvc("GeoSvc", "DCHdigitizer") {
...
StatusCode DCHdigitizer::execute() {
  ...
  for (const auto& input_sim_hit : *input_sim_hits) {
    ...
    const std::string& wireDetElementName = Form("superLayer_%d_layer_%d_phi_%d_wire", m_decoder->get(cellID, "superLayer"), m_decoder->get(cellID, "layer"), m_decoder->get(cellID, "phi"));
    ...
    double simHitGlobalPosition[3] = {input_sim_hit.getPosition().x * dd4hep::mm, input_sim_hit.getPosition().y * dd4hep::mm, input_sim_hit.getPosition().z * dd4hep::mm};
    ...
    debug() << "Global simHit x " << simHitGlobalPosition[0] << " --> Local simHit x " << simHitLocalPosition[0] << " in cm" << endmsg;
    debug() << "Global simHit y " << simHitGlobalPosition[1] << " --> Local simHit y " << simHitLocalPosition[1] << " in cm" << endmsg;
    debug() << "Global simHit z " << simHitGlobalPosition[2] << " --> Local simHit z " << simHitLocalPosition[2] << " in cm" << endmsg;
    // build a vector to easily apply smearing of distance to the wire
    dd4hep::rec::Vector3D simHitLocalPositionVector(simHitLocalPosition[0], simHitLocalPosition[1], simHitLocalPosition[2]);
    ...
    ROOT::Math::Cylindrical3D digiHitLocalPositionVector(smearedDistanceToWire, smearedZ, simHitLocalPositionVector.phi());
    debug() << "Local simHit distance to the wire: " << simHitLocalPositionVector.rho() << " Local digiHit distance to the wire: " << smearedDistanceToWire << " in cm" << endmsg;
    debug() << "Local simHit z: " << simHitLocalPositionVector.z() << " Local digiHit distance to the wire: " << smearedZ << " in cm" << endmsg;
    debug() << "Local simHit phi: " << simHitLocalPositionVector.phi() << " Local digiHit distance to the wire: " << digiHitLocalPositionVector.Phi() << endmsg;
    // go back to the global frame
    double digiHitLocalPosition[3] = {digiHitLocalPositionVector.x(), digiHitLocalPositionVector.y(), digiHitLocalPositionVector.z()};
    double digiHitGlobalPosition[3] = {0, 0, 0};
    wireTransformMatrix.LocalToMaster(digiHitLocalPosition, digiHitGlobalPosition);
    // go back to mm
    edm4hep::Vector3d digiHitGlobalPositionVector(digiHitGlobalPosition[0] / dd4hep::mm, digiHitGlobalPosition[1] / dd4hep::mm, digiHitGlobalPosition[2] / dd4hep::mm);
