 ...
 * studies which require such information, while work on a more accurate dN/dx digitiser on a cell-basis (instead of
 * track-basis) is being conducted.
 *
 *  Inputs:
 *      - TrackMCParticleCollection (tracks for the track length calculation, MCparticle for the number of cluster
 * calculation)
 *      - EventHeaderCollection     (for consistently seeding the random engine)
 *  Properties:
 *      - NAMES of the parameters under which detector dimensions are stored in the XML file, required for the track
 * length calculation (thus, the dimension are loaded dynamically, defaults currently to IDEA drift chamber parameter
 * names)
 *      - Gas selection: Integer to select the gas mixture for which the cluster data is currently available
 *      - Fill factor: A factor to allow the dN/dx calculation for tracks which are not in a continous active tracking
 * volume (e.g. straw tube tracker). The track length is scaled by this factor, thus reducing the number of clusters in
 * the end by the same factor. Outputs:
 *      - RecDqdxCollection: To each track, a quantity dqdx is associated, currently (13.06.2025) with only a value (not
...
private:
  ...
  Gaudi::Property<std::string> m_Zmin_parameter_name{
      ...
      "ZminParameterName",
      {"DCH_gas_Lhalf"},
      "Name of XML file parameter describing the z extent of the active volume (min value in -z direction). For "
      "forward-backward symmetric detectors, use same name as for ZmaxParameterName. This value is then automatically "
      "converted into the negative value."};
  ...
  Gaudi::Property<int> m_GasSel{
      this, "GasSel", {0}, "Gas selection: 0: He(90%)-Isobutane(10%), 1: pure He, 2: Ar(50%)-Ethane(50%), 3: pure Ar."};
  Gaudi::Property<double> m_fill_factor{
      this,
      "FillFactor",
      {1.0},
      "Factor (between 0 and 1) describing the fraction of the detector volume that is active (e.g., for Straw Tube "
      "Tracker the factor is significantly below 1 due to gaps between tubes). The factor is used to scale the "
      "calculated track length and thus the corresponding number of clusters."};
};
