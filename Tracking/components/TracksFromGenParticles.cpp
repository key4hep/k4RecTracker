 ...
 *  It just builds an helix out of the genParticle position, momentum, charge and user defined z component of the
 * (constant) magnetic field. From this helix, different edm4hep::TrackStates (AtIP, AtFirstHit, AtLastHit and
 * AtCalorimeter) are defined. #FIXME for now these trackstates are dummy (copies of the same helix parameters) This is
 * meant to enable technical development needing edm4hep::Track and performance studies where having generator based
 * trackis is a reasonable approximation. Possible inprovement:
 *    - Retrieve magnetic field from geometry: const DD4hep::Field::MagneticField& magneticField = detector.field();
 * DD4hep::DDRec::Vector3D field = magneticField.magneticField(point);
...
struct TracksFromGenParticles final
  ...
  TracksFromGenParticles(const std::string& name, ISvcLocator* svcLoc)
      : MultiTransformer(name, svcLoc, {KeyValues("InputGenParticles", {"MCParticles"})},
  ...
  operator()(const edm4hep::MCParticleCollection& genParticleColl) const override {
