 ...
 *
 *  If the fit fails for a given hypothesis, a fallback track is still generated with chi2 = -1 and ndf = -1 to preserve collection integrity.
        ...
        k4FWCore::MultiTransformer< std::tuple<edm4hep::TrackCollection>(const edm4hep::TrackCollection&)>                                                                         
{
    ...
    std::tuple<edm4hep::TrackCollection> operator()( const edm4hep::TrackCollection& tracks_input) const override                                                 
    {
        ...
        std::vector<edm4hep::TrackCollection> FittedTracksWithFilteredHits;
        std::vector<edm4hep::TrackerHitPlaneCollection> FittedHits;
        ...
        for (const auto& track : tracks_input)
        {
            ...
            if (m_singleEvaluation)
            {
                ...
                if (m_printoutLevel == uint(MSG::DEBUG))  track_init.CovMatrix.Print();
                ...
                bool isFit = track_interface.Fit(m_Beta_init, m_Beta_final, m_Beta_steps, m_Bz, m_printoutLevel);  
            ...
            else
            {
                ...
                else
                {
                    ...
                    track_interface.Fit(m_Beta_init, m_Beta_final, m_Beta_steps, m_Bz, m_printoutLevel);
                    auto edm4hep_track = track_interface.GetTrack_edm4hep();
