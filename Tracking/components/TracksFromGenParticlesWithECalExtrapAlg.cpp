 ...
 *
 *  GaudiAlg version of TracksFromGenParticles, that builds an edm4hep::TrackCollection out of an
 * edm4hep::MCParticleCollection. It just builds an helix out of the genParticle position, momentum, charge and z
 * component of the (constant) magnetic field, retrieved from the detector. From this helix, different
 * edm4hep::TrackStates (AtIP, AtFirstHit, AtLastHit) are defined. The first and last hits are defined as those with
 * smallest and largest time in the input SimTrackerHit collections The algorithm also performs extrapolation to the EM
 * calorimeter inner face is done using the positions of the barrel and endcap retrieved from the detector data
 * extensions. This is meant to enable technical development needing edm4hep::Track and performance studies where having
 * generator based tracks is a reasonable approximation.
...
private:
  ...
  Gaudi::Property<std::vector<std::string>> m_inputSimTrackerHitCollections{
      this, "InputSimTrackerHits", {}, "Names of SimTrackerHit collections to read"};
  ...
  mutable DataHandle<edm4hep::TrackMCParticleLinkCollection> m_links{"TracksFromGenParticlesAlgAssociation",
...
dd4hep::rec::LayeredCalorimeterData* TracksFromGenParticlesWithECalExtrapAlg::getExtension(unsigned int includeFlag,
...
TracksFromGenParticlesWithECalExtrapAlg::TracksFromGenParticlesWithECalExtrapAlg(const std::string& name,
                                                                                 ISvcLocator* svcLoc)
...
StatusCode TracksFromGenParticlesWithECalExtrapAlg::initialize() {
  ...
  for (const auto& col : m_inputSimTrackerHitCollections) {
    ...
    m_inputSimTrackerHitCollectionHandles.push_back(
        new DataHandle<edm4hep::SimTrackerHitCollection>(col, Gaudi::DataHandle::Reader, this));
  }
  ...
  // const dd4hep::rec::LayeredCalorimeterData * wrapperBarrelExtension = getExtension( ( dd4hep::DetType::TRACKER |
  // dd4hep::DetType::BARREL ) , dd4hep::DetType::VERTEX ); const dd4hep::rec::LayeredCalorimeterData *
  // wrapperEndcapExtension = getExtension( ( dd4hep::DetType::TRACKER | dd4hep::DetType::ENDCAP ),
  // dd4hep::DetType::VERTEX ); const dd4hep::rec::LayeredCalorimeterData * vertexBarrelExtension = getExtension( (
  // dd4hep::DetType::TRACKER | dd4hep::DetType::BARREL | dd4hep::DetType::VERTEX ) ); const
  // dd4hep::rec::LayeredCalorimeterData * vertexEndcapExtension = getExtension( ( dd4hep::DetType::TRACKER |
  // dd4hep::DetType::ENDCAP | dd4hep::DetType::VERTEX ) );
...
StatusCode TracksFromGenParticlesWithECalExtrapAlg::execute(const EventContext&) const {
  ...
  for (const auto& genParticle : *genParticleColl) {
    ...
    debug() << "  particle " << iparticle++ << "  PDG: " << genParticle.getPDG()
            << " energy: " << genParticle.getEnergy() << " charge: " << genParticle.getCharge() << endmsg;
    ...
    double genParticleMomentum[] = {genParticle.getMomentum().x, genParticle.getMomentum().y,
                                    genParticle.getMomentum().z};
    ...
    trackState_IP.referencePoint =
        edm4hep::Vector3f((float)genParticleVertex[0], (float)genParticleVertex[1], (float)genParticleVertex[2]);
    ...
    for (size_t ih = 0; ih < m_inputSimTrackerHitCollectionHandles.size(); ih++) {
      auto handle =
          dynamic_cast<DataHandle<edm4hep::SimTrackerHitCollection>*>(m_inputSimTrackerHitCollectionHandles[ih]);
      ...
      for (const auto& hit : *coll) {
        ...
        std::array<double, 7> ahit{
            hit.x(), hit.y(), hit.z(), hit.getMomentum()[0], hit.getMomentum()[1], hit.getMomentum()[2], hit.getTime()};
    ...
    if (!trackHits.empty()) {
      ...
      trackState_AtFirstHit.referencePoint =
          edm4hep::Vector3f((float)posAtFirstHit[0], (float)posAtFirstHit[1], (float)posAtFirstHit[2]);
      ...
      trackState_AtLastHit.referencePoint =
          edm4hep::Vector3f((float)posAtLastHit[0], (float)posAtLastHit[1], (float)posAtLastHit[2]);
      ...
      if (m_eCalBarrelInnerR > 0. || m_eCalEndCapInnerR > 0.) {
        ...
        const pandora::Helix helix(trackState_IP.phi, trackState_IP.D0, trackState_IP.Z0, trackState_IP.omega,
                                   trackState_IP.tanLambda, m_Bz);
        ...
        if (m_eCalEndCapInnerR > 0) {
          (void)helix.GetPointInZ(static_cast<float>(signPz) * m_eCalEndCapInnerZ, referencePoint, bestECalProjection,
                                  minGenericTime);
        ...
        debug() << "Radius at calorimeter: "
                << std::sqrt(posAtCalorimeter[0] * posAtCalorimeter[0] + posAtCalorimeter[1] * posAtCalorimeter[1])
                << endmsg;
        ...
        trackState_AtCalorimeter.referencePoint =
            edm4hep::Vector3f((float)posAtCalorimeter[0], (float)posAtCalorimeter[1], (float)posAtCalorimeter[2]);
        // attach the TrackState to the track
