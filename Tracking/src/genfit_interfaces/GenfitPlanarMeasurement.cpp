...
namespace GenfitInterface {
    ...
    PlanarMeasurement::PlanarMeasurement(const edm4hep::TrackerHitPlane& hit, const int det_idx, const int hit_idx, const int debug_lvl=0) {
        ...
        TVector3 U(
            std::sin(U_theta) * std::cos(U_phi),
            std::sin(U_theta) * std::sin(U_phi),
            std::cos(U_theta)
        );
        U = U.Unit();
        ...
        if (debug_lvl > 0) {
            ...
            std::cout << "U direction          : ("
                    << U.X() << ", "
                    << U.Y() << ", "
                    << U.Z() << ")\n";

            std::cout << "V direction          : ("
                    << V.X() << ", "
                    << V.Y() << ", "
                    << V.Z() << ")\n\n";
